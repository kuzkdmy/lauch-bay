...
import com.demandbase.lauch_bay.service.{ApplicationsServiceLive, ProjectsServiceLive}
...
object MainApp extends App {
  ...
  val appLayer =
    ...
    // service layers
    ProjectsServiceLive.layer >+> ApplicationsServiceLive.layer >+>
    // routes access service layers
    ProjectsRouteServiceLive.layer >+> ApplicationsRouteServiceLive.layer >+>
    // zio-http layers
    ServerOptionsServiceLive.layer >+> EventLoopGroup.auto() >+> ServerChannelFactory.auto
  // format: on

  private type RouteEnv = ApplicationsRoute.Env with ProjectsRoute.Env
  val appProgramResource = for {
    ...
    res <- (Server.port(port) ++ Server.app {
             ...
             val swaggerEndpoints = ApplicationsRoute.swaggerEndpoints ++ ProjectsRoute.swaggerEndpoints
