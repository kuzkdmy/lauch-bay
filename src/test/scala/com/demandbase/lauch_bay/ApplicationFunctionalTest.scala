...
object ApplicationFunctionalTest extends BaseFunTest {
  ...
  override def spec = suite("Application")(
    testM("check crud") {
      (for {
        ...
        for {
          created1   <- c.post(baseUri).body(application1.asJson.noSpaces).send(b).flatMap(toApplication)
          loaded1    <- c.get(baseUri.addPath(application1.id.toString)).send(b).flatMap(toApplication)
          list1      <- c.get(baseUri).send(b).flatMap(toApplicationList)
          created2   <- c.post(baseUri).body(application2.asJson.noSpaces).send(b).flatMap(toApplication)
          loaded2    <- c.get(baseUri.addPath(application2.id.toString)).send(b).flatMap(toApplication)
          list2      <- c.get(baseUri).send(b).flatMap(toApplicationList)
          removeCode <- c.delete(baseUri.addPath(application2.id.toString)).send(b).map(_.code.code)
          list3      <- c.get(baseUri).send(b).flatMap(toApplicationList)
        } yield {
  ...
  def toApplication(resp: Response[String]): Task[ApiApplication] =
    Task.fromEither(parse(resp.body).flatMap(_.as[ApiApplication]))
  def toApplicationList(resp: Response[String]): Task[List[ApiApplication]] =
