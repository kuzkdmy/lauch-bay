...
import {MenuItem, ConfigType} from "../../types/types";
...
interface MenuItemsProps {
    ...
    nestedItems: any[];
    topLevelItem: any;
    addItem: (name: string, type: ConfigType, rows: any[], topLevelItem: any) => void;
    topLevel: boolean;
    isOpened: boolean;
    pl: number;
}
...
const MenuItems: FC<MenuItemsProps> = ({
    ...
    const getCollapse = (item: any) => {
        return (
            <Collapse in={open} timeout="auto" unmountOnExit>
                <MenuItems {...{...item, topLevel: false, pl: item.pl + 2, addItem}}/>
            </Collapse>
        );
    }
    ...
    return (
        <>
            ...
            {nestedItems?.map(nested => getCollapse({...nested, topLevel: false, pl, topLevelItem: {name, type, rows}}))}
        </>
    );
}
